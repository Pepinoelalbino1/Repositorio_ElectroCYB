...
type OrderAction =
  | { type: 'CREATE_ORDER'; payload: Omit<Order, 'id' | 'numeroPedido' | 'fecha' | 'estado' | 'historialEstados'> }
...
const orderReducer = (state: OrderState, action: OrderAction): OrderState => {
  switch (action.type) {
    ...
    case 'UPDATE_ORDER_STATUS': {
      ...
      const updatedCurrentOrder = state.currentOrder?.id === action.payload.orderId
...
const OrderContext = createContext<{
  ...
  createOrder: (orderData: Omit<Order, 'id' | 'numeroPedido' | 'fecha' | 'estado' | 'historialEstados'>) => void;
...
export const OrderProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const createOrder = (orderData: Omit<Order, 'id' | 'numeroPedido' | 'fecha' | 'estado' | 'historialEstados'>) => {
